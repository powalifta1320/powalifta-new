...
function drawLineChart(container, config) {
  ...
  for (let v = tickStart; v <= yHigh; v += niceStep) {
    ...
    line.setAttribute('stroke', '#1d1d22'); line.setAttribute('stroke-width', '1');
    svg.appendChild(line);
    const t = document.createElementNS(SVG_NS, 'text');
    t.setAttribute('x', PAD.left - 10); t.setAttribute('y', yv + 4);
    t.setAttribute('text-anchor', 'end');
    t.setAttribute('fill', '#82828c'); t.setAttribute('font-size', '11');
  ...
  if (goal && goal.value != null) {
    ...
    if (yv >= PAD.top - 5 && yv <= h - PAD.bottom + 5) {
      ...
      line.setAttribute('x1', PAD.left); line.setAttribute('x2', w - PAD.right);
      line.setAttribute('y1', yv); line.setAttribute('y2', yv);
      line.setAttribute('stroke', '#ffb547'); line.setAttribute('stroke-width', '1.5');
      line.setAttribute('stroke-dasharray', '6 4');
      svg.appendChild(line);
      const lbl = document.createElementNS(SVG_NS, 'text');
      lbl.setAttribute('x', w - PAD.right - 4); lbl.setAttribute('y', yv - 6);
      lbl.setAttribute('text-anchor', 'end');
      lbl.setAttribute('fill', '#ffb547'); lbl.setAttribute('font-size', '11');
  ...
  series.forEach((s, sIdx) => {
    ...
    points.forEach((p, pi) => {
      ...
      c.setAttribute('stroke', '#0b0b0c'); c.setAttribute('stroke-width', '2');
      c.style.opacity = '0';
  ...
  cross.setAttribute('stroke', '#56565e'); cross.setAttribute('stroke-width', '1');
  cross.setAttribute('stroke-dasharray', '3 3'); cross.setAttribute('opacity', '0');
  cross.style.pointerEvents = 'none';
  svg.appendChild(cross);

  const tooltip = document.createElement('div');
  tooltip.style.cssText = 'position:absolute; pointer-events:none; opacity:0; background:#16161a; border:1px solid #34343d; border-radius:8px; padding:10px 14px; box-shadow:0 12px 30px rgba(0,0,0,0.45); transition:opacity 0.12s; z-index:10; min-width:140px;';
  ...
  svg.addEventListener('mousemove', e => {
    ...
    let html = '<div style="font-size:0.7rem; color:#82828c; text-transform:uppercase; letter-spacing:0.1em; font-weight:700; margin-bottom:8px;">' +
    ...
    series.forEach(s => {
      ...
      if (d != null && d.y != null) {
        ...
        html += '<div style="display:flex; justify-content:space-between; gap:18px; align-items:center; padding:3px 0;">' +
                  '<span style="display:flex; align-items:center; gap:8px; font-size:0.85rem;"><span style="width:8px; height:8px; border-radius:50%; background:' + s.color + ';"></span><span style="color:#b8b8c0;">' + s.name + '</span></span>' +
                  '<span style="font-family:\'Space Grotesk\',monospace; color:#f4f4f6; font-weight:700; font-variant-numeric: tabular-nums;">' + (Math.round(d.y * 10) / 10) + ' ' + unit + '</span>' +
                '</div>';
      }
    });
    if (!any) {
