...
const SUPABASE_URL = 'https://ztabuzlmbztpzkirgfbe.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inp0YWJ1emxtYnp0cHpraXJnZmJlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc3NDE3MjUsImV4cCI6MjA5MzMxNzcyNX0.rXtAsuaX3VzrQi9CjvfqN3EeL8N3dcpgNTGucUsYhdg';
...
const DB = {
  ...
  async listAthletesForCoach(coachId) {
    const { data, error } = await sb.from('profiles').select('*').eq('user_type', 'athlete').eq('coach_id', coachId);
  ...
  async listInvites(coachId) {
    const { data, error } = await sb.from('invites').select('*').eq('coach_id', coachId);
  ...
  async listProgramsForAthlete(athleteId) {
    const { data, error } = await sb.from('programs').select('*').eq('athlete_id', athleteId);
    if (error) { console.warn('listProgramsForAthlete', error); return []; }
    return (data || []).map(mapDbProgram);
  },
  async listProgramsForCoach(coachId) {
    const { data, error } = await sb.from('programs').select('*').eq('coach_id', coachId);
  ...
  async listTemplates(coachId) {
    const { data, error } = await sb.from('program_templates').select('*').eq('coach_id', coachId);
  ...
  async deleteTemplate(id) {
    const { error } = await sb.from('program_templates').delete().eq('id', id);
    if (error) throw error;
  },

  // ---------- WORKOUT LOGS ----------
  async listLogsForAthlete(athleteId) {
    const { data, error } = await sb.from('workout_logs').select('*').eq('athlete_id', athleteId).order('date', { ascending: true });
  ...
  async listBwForAthlete(athleteId) {
    const { data, error } = await sb.from('bodyweight').select('*').eq('athlete_id', athleteId).order('date', { ascending: true });
  ...
  async listNotesForAthlete(athleteId) {
    const { data, error } = await sb.from('session_notes').select('*').eq('athlete_id', athleteId);
  ...
  async getGoals(athleteId) {
    const { data, error } = await sb.from('goals').select('*').eq('athlete_id', athleteId).maybeSingle();
    if (error) { console.warn('getGoals', error); return null; }
    return data ? mapDbGoals(data) : null;
  },
  ...
  async listRestForAthlete(athleteId) {
    const { data, error } = await sb.from('rest_days').select('*').eq('athlete_id', athleteId);
  ...
  async deleteRest(athleteId, date) {
    const { error } = await sb.from('rest_days').delete().eq('athlete_id', athleteId).eq('date', date);
    if (error) throw error;
  },
