...
Deno.serve(async (req) => {
  try {
    ...
    // Clone the program payload into the buyer's programs table.
    // Strategy: REPLACE any existing program for this athlete with the purchased one
    // (single-program model). We delete first then insert.
    const payloadProgram = program.program_payload as any
    if (payloadProgram && payloadProgram.weeks) {
      // Delete the buyer's existing program (if any) so the purchased one is their active one
      await supabase.from('programs').delete().eq('athlete_id', buyerId)

      // Insert the cloned program. coach_id = the original coach (for credit).
      // athlete_id = the buyer.
      await supabase.from('programs').insert({
        athlete_id: buyerId,
        coach_id: program.coach_id,
        name: program.title,
        weeks: payloadProgram.weeks,
        updated_at: new Date().toISOString()
      })
    }
